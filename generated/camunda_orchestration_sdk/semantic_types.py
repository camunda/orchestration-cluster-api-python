...
class AgentInstanceKey(str):
	def __new__(cls, value: str) -> "AgentInstanceKey":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"AgentInstanceKey must be str, got {type(value).__name__}: {value!r}")
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"AgentInstanceKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
			raise ValueError(f"AgentInstanceKey shorter than minLength 1, got {value!r}")
		if len(value) > 25:
			raise ValueError(f"AgentInstanceKey longer than maxLength 25, got {value!r}")
...
class AuditLogKey(str):
	def __new__(cls, value: str) -> "AuditLogKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"AuditLogKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class AuthorizationKey(str):
	def __new__(cls, value: str) -> "AuthorizationKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"AuthorizationKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class BusinessId(str):
	def __new__(cls, value: str) -> "BusinessId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"BusinessId must be str, got {type(value).__name__}: {value!r}")
		if len(value) < 1:
...
class ClientId(str):
	def __new__(cls, value: str) -> "ClientId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"ClientId must be str, got {type(value).__name__}: {value!r}")
...
class ClusterVariableName(str):
	def __new__(cls, value: str) -> "ClusterVariableName":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"ClusterVariableName must be str, got {type(value).__name__}: {value!r}")
		if re.fullmatch('^[a-zA-Z0-9_~@.+-]+$', value) is None:
			pat = '^[a-zA-Z0-9_~@.+-]+$'
			raise ValueError(f"ClusterVariableName does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
			raise ValueError(f"ClusterVariableName shorter than minLength 1, got {value!r}")
		if len(value) > 256:
			raise ValueError(f"ClusterVariableName longer than maxLength 256, got {value!r}")
		return super().__new__(cls, value)
...
class ConditionalEvaluationKey(str):
	def __new__(cls, value: str) -> "ConditionalEvaluationKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"ConditionalEvaluationKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
			raise ValueError(f"ConditionalEvaluationKey shorter than minLength 1, got {value!r}")
		if len(value) > 25:
			raise ValueError(f"ConditionalEvaluationKey longer than maxLength 25, got {value!r}")
		return super().__new__(cls, value)
...
class DecisionDefinitionId(str):
	def __new__(cls, value: str) -> "DecisionDefinitionId":
		...
		if re.fullmatch('^[\\w_][\\w0-9_\\-\\.]*$', value) is None:
			pat = '^[\\w_][\\w0-9_\\-\\.]*$'
			raise ValueError(f"DecisionDefinitionId does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class DecisionDefinitionKey(str):
	def __new__(cls, value: str) -> "DecisionDefinitionKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"DecisionDefinitionKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class DecisionEvaluationInstanceKey(str):
	def __new__(cls, value: str) -> "DecisionEvaluationInstanceKey":
		...
		if re.fullmatch('^[0-9]+-[0-9]+$', value) is None:
			pat = '^[0-9]+-[0-9]+$'
			raise ValueError(f"DecisionEvaluationInstanceKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 3:
			raise ValueError(f"DecisionEvaluationInstanceKey shorter than minLength 3, got {value!r}")
		if len(value) > 30:
			raise ValueError(f"DecisionEvaluationInstanceKey longer than maxLength 30, got {value!r}")
		return super().__new__(cls, value)
...
class DecisionEvaluationKey(str):
	def __new__(cls, value: str) -> "DecisionEvaluationKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"DecisionEvaluationKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class DecisionInstanceKey(str):
	def __new__(cls, value: str) -> "DecisionInstanceKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"DecisionInstanceKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class DecisionRequirementsKey(str):
	def __new__(cls, value: str) -> "DecisionRequirementsKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"DecisionRequirementsKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class DeploymentKey(str):
	def __new__(cls, value: str) -> "DeploymentKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"DeploymentKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class ElementInstanceKey(str):
	def __new__(cls, value: str) -> "ElementInstanceKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"ElementInstanceKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class EndCursor(str):
	def __new__(cls, value: str) -> "EndCursor":
		...
		if re.fullmatch('^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}(?:==)?|[A-Za-z0-9+/]{3}=)?$', value) is None:
...
class GlobalListenerId(str):
	def __new__(cls, value: str) -> "GlobalListenerId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"GlobalListenerId must be str, got {type(value).__name__}: {value!r}")
		if re.fullmatch('^[a-zA-Z0-9_~@.+\\-]+$', value) is None:
			pat = '^[a-zA-Z0-9_~@.+\\-]+$'
			raise ValueError(f"GlobalListenerId does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
			raise ValueError(f"GlobalListenerId shorter than minLength 1, got {value!r}")
		if len(value) > 256:
			raise ValueError(f"GlobalListenerId longer than maxLength 256, got {value!r}")
		return super().__new__(cls, value)

class GroupId(str):
	def __new__(cls, value: str) -> "GroupId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"GroupId must be str, got {type(value).__name__}: {value!r}")
		if len(value) < 1:
...
class IncidentKey(str):
	def __new__(cls, value: str) -> "IncidentKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"IncidentKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class MappingRuleId(str):
	def __new__(cls, value: str) -> "MappingRuleId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"MappingRuleId must be str, got {type(value).__name__}: {value!r}")
		if re.fullmatch('^[a-zA-Z0-9_~@.+-]+$', value) is None:
			pat = '^[a-zA-Z0-9_~@.+-]+$'
			raise ValueError(f"MappingRuleId does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class MessageKey(str):
	def __new__(cls, value: str) -> "MessageKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"MessageKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class MessageSubscriptionKey(str):
	def __new__(cls, value: str) -> "MessageSubscriptionKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"MessageSubscriptionKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
			raise ValueError(f"MessageSubscriptionKey shorter than minLength 1, got {value!r}")
		if len(value) > 25:
			raise ValueError(f"MessageSubscriptionKey longer than maxLength 25, got {value!r}")
		return super().__new__(cls, value)
...
class ProcessDefinitionId(str):
	def __new__(cls, value: str) -> "ProcessDefinitionId":
		...
		if re.fullmatch('^[\\w_][\\w0-9_\\-\\.]*$', value) is None:
			pat = '^[\\w_][\\w0-9_\\-\\.]*$'
			raise ValueError(f"ProcessDefinitionId does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class ProcessDefinitionKey(str):
	def __new__(cls, value: str) -> "ProcessDefinitionKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"ProcessDefinitionKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class ProcessInstanceKey(str):
	def __new__(cls, value: str) -> "ProcessInstanceKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"ProcessInstanceKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class RoleId(str):
	def __new__(cls, value: str) -> "RoleId":
		if not isinstance(value, str):  # pyright: ignore[reportUnnecessaryIsInstance]
			raise TypeError(f"RoleId must be str, got {type(value).__name__}: {value!r}")
		if re.fullmatch('^[a-zA-Z0-9_~@.+-]+$', value) is None:
			pat = '^[a-zA-Z0-9_~@.+-]+$'
...
class StartCursor(str):
	def __new__(cls, value: str) -> "StartCursor":
		...
		if re.fullmatch('^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}(?:==)?|[A-Za-z0-9+/]{3}=)?$', value) is None:
			pat = '^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}(?:==)?|[A-Za-z0-9+/]{3}=)?$'
			raise ValueError(f"StartCursor does not match pattern {pat!r}, got {value!r}")
		return super().__new__(cls, value)
...
class UserTaskKey(str):
	def __new__(cls, value: str) -> "UserTaskKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"UserTaskKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
class VariableKey(str):
	def __new__(cls, value: str) -> "VariableKey":
		...
		if re.fullmatch('^-?[0-9]+$', value) is None:
			pat = '^-?[0-9]+$'
			raise ValueError(f"VariableKey does not match pattern {pat!r}, got {value!r}")
		if len(value) < 1:
...
def lift_resource_key(value: Any) -> ResourceKey:
	...
	try:
		...
		pass
	raise ValueError(f"ResourceKey: value {value!r} does not match any branch (ProcessDefinitionKey, DecisionRequirementsKey, FormKey, DecisionDefinitionKey)")
...
def lift_scope_key(value: Any) -> ScopeKey:
	...
	try:
		...
		pass
	raise ValueError(f"ScopeKey: value {value!r} does not match any branch (ProcessInstanceKey, ElementInstanceKey)")
