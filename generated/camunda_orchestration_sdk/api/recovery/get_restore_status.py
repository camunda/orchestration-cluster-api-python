...
def _get_kwargs() -> dict[str, Any]:
    _kwargs: dict[str, Any] = {"method": "get", "url": "/restore"}
    return _kwargs
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = RestoreStatusResponse.from_dict(response.json())
        return response_200
...
def sync(*, client: AuthenticatedClient, **kwargs: Any) -> RestoreStatusResponse:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="get_restore_status"
        )
    assert response.parsed is not None
    return cast(RestoreStatusResponse, response.parsed)
