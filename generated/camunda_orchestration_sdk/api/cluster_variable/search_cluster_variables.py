...
from ...models.search_cluster_variables_response_400 import (
    SearchClusterVariablesResponse400,
)
from ...models.search_cluster_variables_response_401 import (
    SearchClusterVariablesResponse401,
)
from ...models.search_cluster_variables_response_403 import (
    SearchClusterVariablesResponse403,
)
from ...models.search_cluster_variables_response_500 import (
...
def _get_kwargs(
    *,
    body: SearchClusterVariablesData | Unset = UNSET,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.SearchClusterVariablesBadRequest(
                ...
                content=response.content,
                parsed=cast(SearchClusterVariablesResponse400, response.parsed),
            )
        if response.status_code == 401:
            raise errors.SearchClusterVariablesUnauthorized(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchClusterVariablesResponse401, response.parsed),
            )
        if response.status_code == 403:
            raise errors.SearchClusterVariablesForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchClusterVariablesResponse403, response.parsed),
            )
        if response.status_code == 500:
            raise errors.SearchClusterVariablesInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchClusterVariablesResponse500, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    assert response.parsed is not None
    return cast(SearchClusterVariablesResponse200, response.parsed)
