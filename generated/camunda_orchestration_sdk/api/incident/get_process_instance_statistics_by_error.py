...
from ...models.get_process_instance_statistics_by_error_response_400 import (
    GetProcessInstanceStatisticsByErrorResponse400,
)
from ...models.get_process_instance_statistics_by_error_response_401 import (
    GetProcessInstanceStatisticsByErrorResponse401,
)
from ...models.get_process_instance_statistics_by_error_response_403 import (
    GetProcessInstanceStatisticsByErrorResponse403,
)
from ...models.get_process_instance_statistics_by_error_response_500 import (
...
def _get_kwargs(
    *, body: GetProcessInstanceStatisticsByErrorData | Unset = UNSET
) -> dict[str, Any]:
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = GetProcessInstanceStatisticsByErrorResponse200.from_dict(
            response.json()
        )
        return response_200
    if response.status_code == 400:
        response_400 = GetProcessInstanceStatisticsByErrorResponse400.from_dict(
            response.json()
        )
        return response_400
    if response.status_code == 401:
        response_401 = GetProcessInstanceStatisticsByErrorResponse401.from_dict(
            response.json()
        )
        return response_401
    if response.status_code == 403:
        response_403 = GetProcessInstanceStatisticsByErrorResponse403.from_dict(
            response.json()
        )
        return response_403
    if response.status_code == 500:
        response_500 = GetProcessInstanceStatisticsByErrorResponse500.from_dict(
            response.json()
        )
        return response_500
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.GetProcessInstanceStatisticsByErrorBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(
                    GetProcessInstanceStatisticsByErrorResponse400, response.parsed
                ),
            )
        if response.status_code == 401:
            raise errors.GetProcessInstanceStatisticsByErrorUnauthorized(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(
                    GetProcessInstanceStatisticsByErrorResponse401, response.parsed
                ),
            )
        if response.status_code == 403:
            raise errors.GetProcessInstanceStatisticsByErrorForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(
                    GetProcessInstanceStatisticsByErrorResponse403, response.parsed
                ),
            )
        if response.status_code == 500:
            raise errors.GetProcessInstanceStatisticsByErrorInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(
                    GetProcessInstanceStatisticsByErrorResponse500, response.parsed
                ),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
