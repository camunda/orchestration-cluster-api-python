...
def sync_detailed(*, client: AuthenticatedClient | Client, body: SearchIncidentsData | Unset=UNSET) -> Response[SearchIncidentsResponse200 | SearchIncidentsResponse400 | SearchIncidentsResponse401 | SearchIncidentsResponse403 | SearchIncidentsResponse500]:
    """Search incidents
...
def sync(*, client: AuthenticatedClient | Client, body: SearchIncidentsData | Unset=UNSET, **kwargs: Any) -> SearchIncidentsResponse200:
    """Search incidents
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.SearchIncidentsBadRequest(status_code=response.status_code, content=response.content, parsed=cast(SearchIncidentsResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.SearchIncidentsUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(SearchIncidentsResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.SearchIncidentsForbidden(status_code=response.status_code, content=response.content, parsed=cast(SearchIncidentsResponse403, response.parsed))
        if response.status_code == 500:
            raise errors.SearchIncidentsInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(SearchIncidentsResponse500, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
    assert response.parsed is not None
    return cast(SearchIncidentsResponse200, response.parsed)

async def asyncio_detailed(*, client: AuthenticatedClient | Client, body: SearchIncidentsData | Unset=UNSET) -> Response[SearchIncidentsResponse200 | SearchIncidentsResponse400 | SearchIncidentsResponse401 | SearchIncidentsResponse403 | SearchIncidentsResponse500]:
    """Search incidents
...
async def asyncio(*, client: AuthenticatedClient | Client, body: SearchIncidentsData | Unset=UNSET, **kwargs: Any) -> SearchIncidentsResponse200:
    """Search incidents
