...
def sync_detailed(
    username: str, *, client: AuthenticatedClient | Client, body: UpdateUserData
) -> Response[
    UpdateUserResponse200
    | UpdateUserResponse400
    | UpdateUserResponse403
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UpdateUserBadRequest(
                ...
                content=response.content,
                parsed=cast(UpdateUserResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.UpdateUserForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateUserResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UpdateUserNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateUserResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UpdateUserInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateUserResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UpdateUserServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateUserResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
