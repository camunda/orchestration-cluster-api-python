...
def sync_detailed(
    *, client: AuthenticatedClient, body: JobWorkerStatisticsQuery
) -> Response[JobWorkerStatisticsQueryResult | ProblemDetail]:
...
def sync(
    *, client: AuthenticatedClient, body: JobWorkerStatisticsQuery, **kwargs: Any
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="get_job_worker_statistics",
            )
        if response.status_code == 401:
