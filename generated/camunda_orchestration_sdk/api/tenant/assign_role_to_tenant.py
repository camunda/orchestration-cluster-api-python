...
def sync(
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.AssignRoleToTenantBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignRoleToTenantResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.AssignRoleToTenantForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignRoleToTenantResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.AssignRoleToTenantNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignRoleToTenantResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.AssignRoleToTenantInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignRoleToTenantResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.AssignRoleToTenantServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignRoleToTenantResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
