...
from ...models.unassign_mapping_rule_from_tenant_response_400 import (
    UnassignMappingRuleFromTenantResponse400,
)
from ...models.unassign_mapping_rule_from_tenant_response_403 import (
    UnassignMappingRuleFromTenantResponse403,
)
from ...models.unassign_mapping_rule_from_tenant_response_404 import (
    UnassignMappingRuleFromTenantResponse404,
)
from ...models.unassign_mapping_rule_from_tenant_response_500 import (
    UnassignMappingRuleFromTenantResponse500,
)
from ...models.unassign_mapping_rule_from_tenant_response_503 import (
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 400:
        response_400 = UnassignMappingRuleFromTenantResponse400.from_dict(
            response.json()
        )
        return response_400
    if response.status_code == 403:
        response_403 = UnassignMappingRuleFromTenantResponse403.from_dict(
            response.json()
        )
        return response_403
    if response.status_code == 404:
        response_404 = UnassignMappingRuleFromTenantResponse404.from_dict(
            response.json()
        )
        return response_404
    if response.status_code == 500:
        response_500 = UnassignMappingRuleFromTenantResponse500.from_dict(
            response.json()
        )
        return response_500
    if response.status_code == 503:
        response_503 = UnassignMappingRuleFromTenantResponse503.from_dict(
            response.json()
        )
        return response_503
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignMappingRuleFromTenantBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignMappingRuleFromTenantResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.UnassignMappingRuleFromTenantForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignMappingRuleFromTenantResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UnassignMappingRuleFromTenantNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignMappingRuleFromTenantResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UnassignMappingRuleFromTenantInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignMappingRuleFromTenantResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UnassignMappingRuleFromTenantServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignMappingRuleFromTenantResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
