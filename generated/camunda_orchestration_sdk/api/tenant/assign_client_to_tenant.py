...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.AssignClientToTenantBadRequest(status_code=response.status_code, content=response.content, parsed=cast(AssignClientToTenantResponse400, response.parsed))
        if response.status_code == 403:
            raise errors.AssignClientToTenantForbidden(status_code=response.status_code, content=response.content, parsed=cast(AssignClientToTenantResponse403, response.parsed))
        if response.status_code == 404:
            raise errors.AssignClientToTenantNotFound(status_code=response.status_code, content=response.content, parsed=cast(AssignClientToTenantResponse404, response.parsed))
        if response.status_code == 500:
            raise errors.AssignClientToTenantInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(AssignClientToTenantResponse500, response.parsed))
        if response.status_code == 503:
            raise errors.AssignClientToTenantServiceUnavailable(status_code=response.status_code, content=response.content, parsed=cast(AssignClientToTenantResponse503, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
