...
from ...models.unassign_group_from_tenant_response_400 import (
    UnassignGroupFromTenantResponse400,
)
from ...models.unassign_group_from_tenant_response_403 import (
    UnassignGroupFromTenantResponse403,
)
from ...models.unassign_group_from_tenant_response_404 import (
    UnassignGroupFromTenantResponse404,
)
from ...models.unassign_group_from_tenant_response_500 import (
    UnassignGroupFromTenantResponse500,
)
from ...models.unassign_group_from_tenant_response_503 import (
...
def _get_kwargs(tenant_id: str, group_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "delete",
        "url": "/tenants/{tenant_id}/groups/{group_id}".format(
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignGroupFromTenantBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignGroupFromTenantResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.UnassignGroupFromTenantForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignGroupFromTenantResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UnassignGroupFromTenantNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignGroupFromTenantResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UnassignGroupFromTenantInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignGroupFromTenantResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UnassignGroupFromTenantServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignGroupFromTenantResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
...
async def asyncio(
    ...
    **kwargs: Any,
    ...
    response = await asyncio_detailed(
        tenant_id=tenant_id, group_id=group_id, client=client
    )
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignGroupFromTenantBadRequest(
