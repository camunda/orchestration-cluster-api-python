...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> ProblemDetail | TenantCreateResult | None:
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[ProblemDetail | TenantCreateResult]:
    return Response(status_code=HTTPStatus(response.status_code), content=response.content, headers=response.headers, parsed=_parse_response(client=client, response=response))

def sync_detailed(*, client: AuthenticatedClient, body: TenantCreateRequest) -> Response[ProblemDetail | TenantCreateResult]:
    """Create tenant
...
def sync(*, client: AuthenticatedClient, body: TenantCreateRequest, **kwargs: Any) -> TenantCreateResult:
    """Create tenant
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        ...
        if response.status_code == 409:
            raise errors.ConflictError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='create_tenant')
        if response.status_code == 500:
...
async def asyncio_detailed(*, client: AuthenticatedClient, body: TenantCreateRequest) -> Response[ProblemDetail | TenantCreateResult]:
    """Create tenant
...
async def asyncio(*, client: AuthenticatedClient, body: TenantCreateRequest, **kwargs: Any) -> TenantCreateResult:
    """Create tenant
