...
from ...models.unassign_client_from_tenant_response_400 import (
    UnassignClientFromTenantResponse400,
)
from ...models.unassign_client_from_tenant_response_403 import (
    UnassignClientFromTenantResponse403,
)
from ...models.unassign_client_from_tenant_response_404 import (
    UnassignClientFromTenantResponse404,
)
from ...models.unassign_client_from_tenant_response_500 import (
    UnassignClientFromTenantResponse500,
)
from ...models.unassign_client_from_tenant_response_503 import (
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignClientFromTenantBadRequest(
                ...
                content=response.content,
                parsed=cast(UnassignClientFromTenantResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.UnassignClientFromTenantForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignClientFromTenantResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UnassignClientFromTenantNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignClientFromTenantResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UnassignClientFromTenantInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignClientFromTenantResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UnassignClientFromTenantServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignClientFromTenantResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
