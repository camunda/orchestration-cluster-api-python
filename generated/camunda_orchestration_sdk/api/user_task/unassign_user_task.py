...
def sync_detailed(user_task_key: str, *, client: AuthenticatedClient) -> Response[Any | ProblemDetail]:
    """Unassign user task
...
def sync(user_task_key: str, *, client: AuthenticatedClient, **kwargs: Any) -> None:
    """Unassign user task
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        if response.status_code == 404:
            raise errors.NotFoundError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        if response.status_code == 409:
            raise errors.ConflictError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        if response.status_code == 500:
            raise errors.InternalServerErrorError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        if response.status_code == 503:
            raise errors.ServiceUnavailableError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        if response.status_code == 504:
            raise errors.GatewayTimeoutError(status_code=response.status_code, content=response.content, parsed=cast(ProblemDetail, response.parsed), operation_id='unassign_user_task')
        raise errors.UnexpectedStatus(response.status_code, response.content, operation_id='unassign_user_task')
    return None

async def asyncio_detailed(user_task_key: str, *, client: AuthenticatedClient) -> Response[Any | ProblemDetail]:
    """Unassign user task
...
async def asyncio(user_task_key: str, *, client: AuthenticatedClient, **kwargs: Any) -> None:
    """Unassign user task
