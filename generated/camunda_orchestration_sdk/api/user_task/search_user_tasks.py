...
def sync(*, client: AuthenticatedClient | Client, body: SearchUserTasksData | Unset=UNSET, **kwargs: Any) -> SearchUserTasksResponse200:
    """Search user tasks
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.SearchUserTasksBadRequest(status_code=response.status_code, content=response.content, parsed=cast(SearchUserTasksResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.SearchUserTasksUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(SearchUserTasksResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.SearchUserTasksForbidden(status_code=response.status_code, content=response.content, parsed=cast(SearchUserTasksResponse403, response.parsed))
        if response.status_code == 500:
            raise errors.SearchUserTasksInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(SearchUserTasksResponse500, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: SearchUserTasksData | Unset=UNSET, **kwargs: Any) -> SearchUserTasksResponse200:
    """Search user tasks
