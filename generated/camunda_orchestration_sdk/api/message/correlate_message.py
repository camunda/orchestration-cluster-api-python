...
def sync(*, client: AuthenticatedClient | Client, body: CorrelateMessageData, **kwargs: Any) -> CorrelateMessageResponse200:
    """Correlate message
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.CorrelateMessageBadRequest(status_code=response.status_code, content=response.content, parsed=cast(CorrelateMessageResponse400, response.parsed))
        if response.status_code == 403:
            raise errors.CorrelateMessageForbidden(status_code=response.status_code, content=response.content, parsed=cast(CorrelateMessageResponse403, response.parsed))
        if response.status_code == 404:
            raise errors.CorrelateMessageNotFound(status_code=response.status_code, content=response.content, parsed=cast(CorrelateMessageResponse404, response.parsed))
        if response.status_code == 500:
            raise errors.CorrelateMessageInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(CorrelateMessageResponse500, response.parsed))
        if response.status_code == 503:
            raise errors.CorrelateMessageServiceUnavailable(status_code=response.status_code, content=response.content, parsed=cast(CorrelateMessageResponse503, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: CorrelateMessageData, **kwargs: Any) -> CorrelateMessageResponse200:
    """Correlate message
