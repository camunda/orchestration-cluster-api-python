...
def sync(*, client: AuthenticatedClient | Client, body: SearchDecisionInstancesData | Unset=UNSET, **kwargs: Any) -> SearchDecisionInstancesResponse200:
    """Search decision instances
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.SearchDecisionInstancesBadRequest(status_code=response.status_code, content=response.content, parsed=cast(SearchDecisionInstancesResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.SearchDecisionInstancesUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(SearchDecisionInstancesResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.SearchDecisionInstancesForbidden(status_code=response.status_code, content=response.content, parsed=cast(SearchDecisionInstancesResponse403, response.parsed))
        if response.status_code == 500:
            raise errors.SearchDecisionInstancesInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(SearchDecisionInstancesResponse500, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: SearchDecisionInstancesData | Unset=UNSET, **kwargs: Any) -> SearchDecisionInstancesResponse200:
    """Search decision instances
