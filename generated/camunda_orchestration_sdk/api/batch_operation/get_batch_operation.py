    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.GetBatchOperationBadRequest(
                ...
                content=response.content,
                parsed=cast(GetBatchOperationResponse400, response.parsed),
            )
        if response.status_code == 404:
            raise errors.GetBatchOperationNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(GetBatchOperationResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.GetBatchOperationInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(GetBatchOperationResponse500, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
