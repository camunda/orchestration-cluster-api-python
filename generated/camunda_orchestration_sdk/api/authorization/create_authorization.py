...
def sync(*, client: AuthenticatedClient | Client, body: Object | Object1, **kwargs: Any) -> CreateAuthorizationResponse201:
    """Create authorization
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.CreateAuthorizationBadRequest(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.CreateAuthorizationUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.CreateAuthorizationForbidden(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse403, response.parsed))
        if response.status_code == 404:
            raise errors.CreateAuthorizationNotFound(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse404, response.parsed))
        if response.status_code == 500:
            raise errors.CreateAuthorizationInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse500, response.parsed))
        if response.status_code == 503:
            raise errors.CreateAuthorizationServiceUnavailable(status_code=response.status_code, content=response.content, parsed=cast(CreateAuthorizationResponse503, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: Object | Object1, **kwargs: Any) -> CreateAuthorizationResponse201:
    """Create authorization
