...
def sync_detailed(
    *, client: AuthenticatedClient, body: ConditionalEvaluationInstruction
) -> Response[EvaluateConditionalResult | ProblemDetail]:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="evaluate_conditionals",
            )
        if response.status_code == 403:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="evaluate_conditionals"
        )
