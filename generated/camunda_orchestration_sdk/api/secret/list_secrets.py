...
def _get_kwargs(*, body: SecretListRequest | Unset = UNSET) -> dict[str, Any]:
    ...
    _kwargs: dict[str, Any] = {"method": "post", "url": "/secrets/list"}
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = SecretListResult.from_dict(response.json())
        return response_200
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="list_secrets"
        )
    assert response.parsed is not None
    return cast(SecretListResult, response.parsed)
