...
def sync_detailed(
    mapping_rule_id: str, *, client: AuthenticatedClient
) -> Response[Any | ProblemDetail]:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 401:
            raise errors.UnauthorizedError(
                ...
                operation_id="delete_mapping_rule",
            )
        if response.status_code == 404:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="delete_mapping_rule"
        )
    return None
...
async def asyncio(
    mapping_rule_id: str, *, client: AuthenticatedClient, **kwargs: Any
) -> None:
    """Delete a mapping rule
