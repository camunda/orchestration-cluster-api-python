...
def sync_detailed(
    *, client: AuthenticatedClient | Client, body: CreateMappingRuleData | Unset = UNSET
) -> Response[
    CreateMappingRuleResponse201
    | CreateMappingRuleResponse400
    | CreateMappingRuleResponse403
...
def sync(
    *,
    ...
    body: CreateMappingRuleData | Unset = UNSET,
    **kwargs: Any,
) -> CreateMappingRuleResponse201:
    """Create mapping rule
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.CreateMappingRuleBadRequest(
                ...
                content=response.content,
                parsed=cast(CreateMappingRuleResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.CreateMappingRuleForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(CreateMappingRuleResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.CreateMappingRuleNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(CreateMappingRuleResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.CreateMappingRuleInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(CreateMappingRuleResponse500, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
