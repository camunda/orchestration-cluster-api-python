...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/agent-instances/{agent_instance_key}/history".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 201:
        response_201 = AgentInstanceHistoryItemCreationResult.from_dict(response.json())
        return response_201
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="create_agent_instance_history_item",
            )
        if response.status_code == 401:
    ...
    return cast(AgentInstanceHistoryItemCreationResult, response.parsed)
