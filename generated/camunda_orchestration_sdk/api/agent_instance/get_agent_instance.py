...
def sync_detailed(
    agent_instance_key: str, *, client: AuthenticatedClient | Client
) -> Response[AgentInstanceResult | ProblemDetail]:
    """Get agent instance
...
def sync(
    agent_instance_key: str, *, client: AuthenticatedClient | Client, **kwargs: Any
) -> AgentInstanceResult:
    """Get agent instance
    ...
    if response.status_code < 200 or response.status_code >= 300:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="get_agent_instance"
        )
...
async def asyncio(
    ...
    response = await asyncio_detailed(
        agent_instance_key=agent_instance_key, client=client
    )
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
