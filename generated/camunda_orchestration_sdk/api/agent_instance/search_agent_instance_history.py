...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/agent-instances/{agent_instance_key}/history/search".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = AgentInstanceHistorySearchQueryResult.from_dict(response.json())
        return response_200
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="search_agent_instance_history",
            )
        if response.status_code == 401:
    ...
    return cast(AgentInstanceHistorySearchQueryResult, response.parsed)
