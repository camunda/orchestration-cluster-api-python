...
def sync(*, client: AuthenticatedClient | Client, body: CreateDocumentsData, store_id: str | Unset=UNSET, **kwargs: Any) -> CreateDocumentsResponse201:
    """Upload multiple documents
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.CreateDocumentsBadRequest(status_code=response.status_code, content=response.content, parsed=cast(CreateDocumentsResponse400, response.parsed))
        if response.status_code == 415:
            raise errors.CreateDocumentsUnsupportedMediaType(status_code=response.status_code, content=response.content, parsed=cast(CreateDocumentsResponse415, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: CreateDocumentsData, store_id: str | Unset=UNSET, **kwargs: Any) -> CreateDocumentsResponse201:
    """Upload multiple documents
