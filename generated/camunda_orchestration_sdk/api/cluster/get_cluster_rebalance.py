...
def _get_kwargs() -> dict[str, Any]:
    _kwargs: dict[str, Any] = {"method": "get", "url": "/cluster/v2/rebalance"}
    return _kwargs
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = ClusterBalanceResponse.from_dict(response.json())
        return response_200
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 401:
            raise errors.UnauthorizedError(
                ...
                operation_id="get_cluster_rebalance",
            )
        if response.status_code == 500:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="get_cluster_rebalance"
        )
