...
def _get_kwargs() -> dict[str, Any]:
    _kwargs: dict[str, Any] = {"method": "post", "url": "/backups/runtime/state/sync"}
    return _kwargs
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = RuntimeBackupState.from_dict(response.json())
        return response_200
...
def sync(*, client: AuthenticatedClient, **kwargs: Any) -> RuntimeBackupState:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 401:
            raise errors.UnauthorizedError(
                ...
                operation_id="sync_runtime_backup_state",
            )
        if response.status_code == 403:
    ...
    return cast(RuntimeBackupState, response.parsed)
...
async def asyncio(*, client: AuthenticatedClient, **kwargs: Any) -> RuntimeBackupState:
    """Force-write runtime backup state
