...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200: list[BackupInfo] = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = BackupInfo.from_dict(response_200_item_data)
...
def sync_detailed(
    *, client: AuthenticatedClient, prefix: str | Unset = UNSET
    ...
    Returns:
        Response[ProblemDetail | list[BackupInfo]]
    """
    kwargs = _get_kwargs(prefix=prefix)
...
def sync(
    *, client: AuthenticatedClient, prefix: str | Unset = UNSET, **kwargs: Any
    ...
    Returns:
        list[Any]"""
    response = sync_detailed(client=client, prefix=prefix)
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="list_runtime_backups",
            )
        if response.status_code == 401:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="list_runtime_backups"
        )
...
async def asyncio(
    *, client: AuthenticatedClient, prefix: str | Unset = UNSET, **kwargs: Any
    ...
    Returns:
        list[Any]"""
    response = await asyncio_detailed(client=client, prefix=prefix)
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                status_code=response.status_code,
