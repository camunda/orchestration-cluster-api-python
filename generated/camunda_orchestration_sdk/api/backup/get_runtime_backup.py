...
def _get_kwargs(backup_id: int) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/backups/runtime/{backup_id}".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = BackupInfo.from_dict(response.json())
        return response_200
...
def sync(backup_id: int, *, client: AuthenticatedClient, **kwargs: Any) -> BackupInfo:
    """Get runtime backup
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 401:
            raise errors.UnauthorizedError(
                ...
                operation_id="get_runtime_backup",
            )
        if response.status_code == 403:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="get_runtime_backup"
        )
    assert response.parsed is not None
    return cast(BackupInfo, response.parsed)
