...
def _get_kwargs(*, body: TakeRuntimeBackupRequest | Unset = UNSET) -> dict[str, Any]:
    headers: dict[str, Any] = {}
    _kwargs: dict[str, Any] = {"method": "post", "url": "/backups/runtime"}
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 202:
        response_202 = TakeRuntimeBackupResponse.from_dict(response.json())
        return response_202
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="take_runtime_backup",
            )
        if response.status_code == 401:
        ...
        raise errors.UnexpectedStatus(
            response.status_code, response.content, operation_id="take_runtime_backup"
        )
    assert response.parsed is not None
    return cast(TakeRuntimeBackupResponse, response.parsed)
