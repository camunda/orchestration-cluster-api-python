    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.SearchUsersForRoleBadRequest(
                ...
                content=response.content,
                parsed=cast(SearchUsersForRoleResponse400, response.parsed),
            )
        if response.status_code == 401:
            raise errors.SearchUsersForRoleUnauthorized(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchUsersForRoleResponse401, response.parsed),
            )
        if response.status_code == 403:
            raise errors.SearchUsersForRoleForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchUsersForRoleResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.SearchUsersForRoleNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchUsersForRoleResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.SearchUsersForRoleInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(SearchUsersForRoleResponse500, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
