...
from ...models.unassign_role_from_mapping_rule_response_400 import (
    UnassignRoleFromMappingRuleResponse400,
)
from ...models.unassign_role_from_mapping_rule_response_403 import (
    UnassignRoleFromMappingRuleResponse403,
)
from ...models.unassign_role_from_mapping_rule_response_404 import (
    UnassignRoleFromMappingRuleResponse404,
)
from ...models.unassign_role_from_mapping_rule_response_500 import (
    UnassignRoleFromMappingRuleResponse500,
)
from ...models.unassign_role_from_mapping_rule_response_503 import (
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignRoleFromMappingRuleBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignRoleFromMappingRuleResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.UnassignRoleFromMappingRuleForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignRoleFromMappingRuleResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UnassignRoleFromMappingRuleNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignRoleFromMappingRuleResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UnassignRoleFromMappingRuleInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignRoleFromMappingRuleResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UnassignRoleFromMappingRuleServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UnassignRoleFromMappingRuleResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
