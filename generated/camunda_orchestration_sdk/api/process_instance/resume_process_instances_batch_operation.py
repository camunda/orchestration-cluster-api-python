...
def _get_kwargs(
    *, body: ProcessInstanceResumptionBatchOperationRequest
    ...
    _kwargs: dict[str, Any] = {"method": "post", "url": "/process-instances/resumption"}
...
def sync_detailed(
    *, client: AuthenticatedClient, body: ProcessInstanceResumptionBatchOperationRequest
) -> Response[BatchOperationCreatedResult | ProblemDetail]:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="resume_process_instances_batch_operation",
            )
        if response.status_code == 401:
