...
def _get_kwargs(
    ...
    *,
    body: None | SuspendProcessInstanceRequest | Unset = UNSET,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/process-instances/{process_instance_key}/suspension".format(
            process_instance_key=quote(str(process_instance_key), safe="")
        ),
    }
    if isinstance(body, SuspendProcessInstanceRequest):
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.BadRequestError(
                ...
                operation_id="suspend_process_instance",
            )
        if response.status_code == 404:
