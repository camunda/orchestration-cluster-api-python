...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UnassignClientFromGroupBadRequest(status_code=response.status_code, content=response.content, parsed=cast(UnassignClientFromGroupResponse400, response.parsed))
        if response.status_code == 403:
            raise errors.UnassignClientFromGroupForbidden(status_code=response.status_code, content=response.content, parsed=cast(UnassignClientFromGroupResponse403, response.parsed))
        if response.status_code == 404:
            raise errors.UnassignClientFromGroupNotFound(status_code=response.status_code, content=response.content, parsed=cast(UnassignClientFromGroupResponse404, response.parsed))
        if response.status_code == 500:
            raise errors.UnassignClientFromGroupInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(UnassignClientFromGroupResponse500, response.parsed))
        if response.status_code == 503:
            raise errors.UnassignClientFromGroupServiceUnavailable(status_code=response.status_code, content=response.content, parsed=cast(UnassignClientFromGroupResponse503, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
