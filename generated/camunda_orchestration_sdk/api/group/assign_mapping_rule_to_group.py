...
from ...models.assign_mapping_rule_to_group_response_400 import (
    AssignMappingRuleToGroupResponse400,
)
from ...models.assign_mapping_rule_to_group_response_403 import (
    AssignMappingRuleToGroupResponse403,
)
from ...models.assign_mapping_rule_to_group_response_404 import (
    AssignMappingRuleToGroupResponse404,
)
from ...models.assign_mapping_rule_to_group_response_409 import (
    AssignMappingRuleToGroupResponse409,
)
from ...models.assign_mapping_rule_to_group_response_500 import (
    AssignMappingRuleToGroupResponse500,
)
from ...models.assign_mapping_rule_to_group_response_503 import (
...
def sync(
    ...
    **kwargs: Any,
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.AssignMappingRuleToGroupBadRequest(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse400, response.parsed),
            )
        if response.status_code == 403:
            raise errors.AssignMappingRuleToGroupForbidden(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse403, response.parsed),
            )
        if response.status_code == 404:
            raise errors.AssignMappingRuleToGroupNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse404, response.parsed),
            )
        if response.status_code == 409:
            raise errors.AssignMappingRuleToGroupConflict(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse409, response.parsed),
            )
        if response.status_code == 500:
            raise errors.AssignMappingRuleToGroupInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.AssignMappingRuleToGroupServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(AssignMappingRuleToGroupResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    return None
