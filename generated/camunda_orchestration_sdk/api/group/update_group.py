...
def sync_detailed(
    group_id: str, *, client: AuthenticatedClient | Client, body: UpdateGroupData
) -> Response[
    UpdateGroupResponse200
    | UpdateGroupResponse400
    | UpdateGroupResponse401
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.UpdateGroupBadRequest(
                ...
                content=response.content,
                parsed=cast(UpdateGroupResponse400, response.parsed),
            )
        if response.status_code == 401:
            raise errors.UpdateGroupUnauthorized(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateGroupResponse401, response.parsed),
            )
        if response.status_code == 404:
            raise errors.UpdateGroupNotFound(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateGroupResponse404, response.parsed),
            )
        if response.status_code == 500:
            raise errors.UpdateGroupInternalServerError(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateGroupResponse500, response.parsed),
            )
        if response.status_code == 503:
            raise errors.UpdateGroupServiceUnavailable(
                status_code=response.status_code,
                content=response.content,
                parsed=cast(UpdateGroupResponse503, response.parsed),
            )
        raise errors.UnexpectedStatus(response.status_code, response.content)
    assert response.parsed is not None
    return cast(UpdateGroupResponse200, response.parsed)
