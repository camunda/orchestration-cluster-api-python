...
def _get_kwargs(*, body: GetProcessDefinitionInstanceStatisticsData | Unset=UNSET) -> dict[str, Any]:
...
def sync(*, client: AuthenticatedClient | Client, body: GetProcessDefinitionInstanceStatisticsData | Unset=UNSET, **kwargs: Any) -> GetProcessDefinitionInstanceStatisticsResponse200:
    """Get process instance statistics
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.GetProcessDefinitionInstanceStatisticsBadRequest(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionInstanceStatisticsResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.GetProcessDefinitionInstanceStatisticsUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionInstanceStatisticsResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.GetProcessDefinitionInstanceStatisticsForbidden(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionInstanceStatisticsResponse403, response.parsed))
        if response.status_code == 500:
            raise errors.GetProcessDefinitionInstanceStatisticsInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionInstanceStatisticsResponse500, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: GetProcessDefinitionInstanceStatisticsData | Unset=UNSET, **kwargs: Any) -> GetProcessDefinitionInstanceStatisticsResponse200:
    """Get process instance statistics
