...
def sync(*, client: AuthenticatedClient | Client, body: GetProcessDefinitionMessageSubscriptionStatisticsData | Unset=UNSET, **kwargs: Any) -> GetProcessDefinitionMessageSubscriptionStatisticsResponse200:
    """Get message subscription statistics
...
Returns:
    ...
    if response.status_code < 200 or response.status_code >= 300:
        if response.status_code == 400:
            raise errors.GetProcessDefinitionMessageSubscriptionStatisticsBadRequest(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionMessageSubscriptionStatisticsResponse400, response.parsed))
        if response.status_code == 401:
            raise errors.GetProcessDefinitionMessageSubscriptionStatisticsUnauthorized(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionMessageSubscriptionStatisticsResponse401, response.parsed))
        if response.status_code == 403:
            raise errors.GetProcessDefinitionMessageSubscriptionStatisticsForbidden(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionMessageSubscriptionStatisticsResponse403, response.parsed))
        if response.status_code == 500:
            raise errors.GetProcessDefinitionMessageSubscriptionStatisticsInternalServerError(status_code=response.status_code, content=response.content, parsed=cast(GetProcessDefinitionMessageSubscriptionStatisticsResponse500, response.parsed))
        raise errors.UnexpectedStatus(response.status_code, response.content)
    assert response.parsed is not None
    return cast(GetProcessDefinitionMessageSubscriptionStatisticsResponse200, response.parsed)
...
async def asyncio(*, client: AuthenticatedClient | Client, body: GetProcessDefinitionMessageSubscriptionStatisticsData | Unset=UNSET, **kwargs: Any) -> GetProcessDefinitionMessageSubscriptionStatisticsResponse200:
    """Get message subscription statistics
