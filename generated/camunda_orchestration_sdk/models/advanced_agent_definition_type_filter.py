...
class AdvancedAgentDefinitionTypeFilter:
    r"""Advanced AgentDefinitionTypeEnum filter.

    Attributes:
        eq (AdvancedAgentDefinitionTypeFilterEq | Unset): Checks for equality with the provided value.
        neq (AdvancedAgentDefinitionTypeFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[AgentDefinitionTypeEnum] | Unset): Checks if the property matches any of the provided values.
        not_in (list[AgentDefinitionTypeEnum] | Unset): Checks if the property matches none of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedAgentDefinitionTypeFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedAgentDefinitionTypeFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedAgentDefinitionTypeFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedAgentDefinitionTypeFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = AgentDefinitionTypeEnum(in_item_data)
        ...
        if _not_in is not UNSET:
            ...
            for not_in_item_data in _not_in:
                not_in_item = AgentDefinitionTypeEnum(not_in_item_data)

                not_in.append(not_in_item)

        like = d.pop("$like", UNSET)

        advanced_agent_definition_type_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_agent_definition_type_filter.additional_properties = d
        return advanced_agent_definition_type_filter
