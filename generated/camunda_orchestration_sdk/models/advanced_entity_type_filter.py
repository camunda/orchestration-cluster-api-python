...
class AdvancedEntityTypeFilter:
    ...
    Attributes:
        eq (AdvancedEntityTypeFilterEq | Unset): Checks for equality with the provided value.
        neq (AdvancedEntityTypeFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[AuditLogEntityTypeEnum] | Unset): Checks if the property matches any of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedEntityTypeFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedEntityTypeFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedEntityTypeFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedEntityTypeFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = AuditLogEntityTypeEnum(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        advanced_entity_type_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_entity_type_filter.additional_properties = d
        return advanced_entity_type_filter
