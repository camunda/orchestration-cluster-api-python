...
class AdvancedWaitStateTypeFilter:
    r"""Advanced wait state type filter.

    Attributes:
        eq (AdvancedWaitStateTypeFilterEq | Unset): Checks for equality with the provided value.
        neq (AdvancedWaitStateTypeFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[WaitStateTypeEnum] | Unset): Checks if the property matches any of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedWaitStateTypeFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedWaitStateTypeFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedWaitStateTypeFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedWaitStateTypeFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = WaitStateTypeEnum(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        advanced_wait_state_type_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_wait_state_type_filter.additional_properties = d
        return advanced_wait_state_type_filter
