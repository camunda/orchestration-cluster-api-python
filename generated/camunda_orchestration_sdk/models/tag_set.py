...
class TagSet(RootModel[list[Tag]]):
	...
	def _validate_array(cls, v: list[Tag]) -> list[Tag]:
		if len(v) > 10: raise ValueError('maxItems 10')
		...
		for _i, _x in enumerate(v):
			...
			if len(str(_x)) < 1: raise ValueError(f'item {_i} minLength 1')
			if len(str(_x)) > 100: raise ValueError(f'item {_i} maxLength 100')
		return v
