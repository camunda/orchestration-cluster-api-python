...
class AdvancedJobStateFilter:
    ...
    Attributes:
        eq (AdvancedJobStateFilterEq | Unset): Checks for equality with the provided value.
        neq (AdvancedJobStateFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[JobStateEnum] | Unset): Checks if the property matches any of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedJobStateFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedJobStateFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedJobStateFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedJobStateFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = JobStateEnum(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        advanced_job_state_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_job_state_filter.additional_properties = d
        return advanced_job_state_filter
