...
from ..models.advanced_agent_instance_status_filter_eq import (
    AdvancedAgentInstanceStatusFilterEq,
)
from ..models.advanced_agent_instance_status_filter_neq import (
...
class AdvancedAgentInstanceStatusFilter:
    r"""Advanced AgentInstanceStatusEnum filter.

    Attributes:
        eq (AdvancedAgentInstanceStatusFilterEq | Unset): Checks for equality with the provided value.
        neq (AdvancedAgentInstanceStatusFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[AgentInstanceStatusEnum] | Unset): Checks if the property matches any of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedAgentInstanceStatusFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedAgentInstanceStatusFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedAgentInstanceStatusFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedAgentInstanceStatusFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = AgentInstanceStatusEnum(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        advanced_agent_instance_status_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_agent_instance_status_filter.additional_properties = d
        return advanced_agent_instance_status_filter
