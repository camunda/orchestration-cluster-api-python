...
class AdvancedActorTypeFilter:
    ...
    Attributes:
        ...
        neq (AdvancedActorTypeFilterNeq | Unset): Checks for inequality with the provided value.
        exists (bool | Unset): Checks if the current property exists.
        in_ (list[AuditLogActorTypeEnum] | Unset): Checks if the property matches any of the provided values.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        _eq = d.pop("$eq", UNSET)
        eq: AdvancedActorTypeFilterEq | Unset
        if isinstance(_eq, Unset):
            eq = UNSET
        else:
            eq = AdvancedActorTypeFilterEq(_eq)

        _neq = d.pop("$neq", UNSET)
        neq: AdvancedActorTypeFilterNeq | Unset
        if isinstance(_neq, Unset):
            neq = UNSET
        else:
            neq = AdvancedActorTypeFilterNeq(_neq)
        ...
        if _in_ is not UNSET:
            ...
            for in_item_data in _in_:
                in_item = AuditLogActorTypeEnum(in_item_data)

                in_.append(in_item)

        like = d.pop("$like", UNSET)

        advanced_actor_type_filter = cls(
            eq=eq,
            neq=neq,
            exists=exists,
            in_=in_,
        ...
        advanced_actor_type_filter.additional_properties = d
        return advanced_actor_type_filter
