...
If an api-reference/ subdirectory exists in the section directory, a
_category_.json is generated for it.

The landing page is written as a sibling of the section directory so that
Docusaurus autogenerated sidebar items don't duplicate the category link:

    output_dir/
      python-sdk.md              ← landing page (category link target)
      python-sdk/                ← autogenerated sidebar scans this dir
...
# page directory.
# Landing page lives at apis-tools/python-sdk.md (depth 1).
# Section pages live at apis-tools/python-sdk/<slug>.md (depth 2).
...
def rewrite_camunda_docs_links(content: str, depth: int = _SECTION_PAGE_DEPTH) -> str:
...
def generate_pages(readme_path: Path, output_dir: Path) -> None:
    ...
    # Section pages and api-reference live inside a python-sdk/ subdirectory.
    section_dir = output_dir / "python-sdk"
    section_dir.mkdir(parents=True, exist_ok=True)

    # --- Landing page: sibling of the section directory ---
    # Uses depth 1 because it lives at apis-tools/python-sdk.md
    landing_preamble = rewrite_camunda_docs_links(preamble, depth=_LANDING_PAGE_DEPTH)
    landing_fm = make_frontmatter("python-sdk", "Python SDK", "Python SDK", 1)
    landing_path = output_dir / "python-sdk.md"
    landing_path.write_text(landing_fm + landing_preamble.strip() + "\n")
    print(f"Generated landing page: {landing_path}")

    # --- Section pages: one per H2, inside python-sdk/ ---
    # Uses depth 2 because they live at apis-tools/python-sdk/<slug>.md
    for i, (title, body) in enumerate(sections):
        slug = slugify(title)
        position = i + 2  # landing page is 1, sections start at 2
        section_content = rewrite_camunda_docs_links(body, depth=_SECTION_PAGE_DEPTH)
        page_content = promote_headings(section_content).strip() + "\n"
