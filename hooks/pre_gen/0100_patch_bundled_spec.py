...
class InlineSchemaExtractor:
    def __init__(self, spec: dict[str, Any]) -> None:
        ...
        self.new_schemas: dict[
            str, Any
        ] = {}  # Store new schemas here to avoid runtime modification errors
    ...
    def process_composition(
        self, node: dict[str, Any], key: str, name_hint: str
    ) -> None:
        ...
        for option_raw in options_list:
            ...
            if isinstance(option_raw, dict):
                ...
                should_extract = "$ref" not in option and (
                    "properties" in option  # inline object
                    or (
                        "enum" in option
                        and ("title" in option or "description" in option)
                    )  # enum with metadata
                    or (
                        "type" in option and len(option) > 2
                    )  # complex type with multiple constraints
                )
            ...
            if should_extract:
                ...
                if fingerprint in self.schema_fingerprints:
                    ...
                    counter: int = 1
                    while (
                        schema_name in self.schemas or schema_name in self.new_schemas
                    ):
    ...
    def flatten_schema(self, schema: SpecNode) -> SpecNode:
        ...
        for k, v in schema.items():
            ...
            if isinstance(v, dict):
                ...
                items_list = cast(list[Any], v)
                new_schema[k] = [
                    self.flatten_schema(cast(dict[str, Any], i))
...
def extract_nested_properties(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    def extract_object_schema(
        obj_schema: dict[str, Any], base_name: str
    ) -> dict[str, str]:
        ...
        if fingerprint in existing_fingerprints:
            ...
            return {
                "$ref": f"#/components/schemas/{existing_fingerprints[fingerprint]}"
            }
        ...
        if "items" in schema and isinstance(schema["items"], dict):
            ...
            if items.get("type") == "object" and (
                ...
                or (
                    ...
                    and items["additionalProperties"] is not False
                )
            ):
    ...
    def process_properties(props: dict[str, Any], parent_name: str) -> dict[str, Any]:
        ...
        for prop_name, prop_schema in props.items():
            ...
            if is_dict:
                ...
                should_extract = (
                    "type" in prop_dict
                    and prop_dict.get("type") == "object"
                    and (
                        "properties" in prop_dict
                        or (
                            "additionalProperties" in prop_dict
                            and prop_dict["additionalProperties"] is not False
                        )
                    )
                )
            ...
            if should_extract:
                ...
                new_props[prop_name] = extract_object_schema(
                    cast(dict[str, Any], prop_schema), base_name
                )
            elif isinstance(prop_schema, dict):
                # Recursively process the property
                clean_prop_name = to_pascal_case(prop_name_str)
                new_props[prop_name] = process_schema(
                    cast(dict[str, Any], prop_schema), f"{parent_name}{clean_prop_name}"
                )
            else:
                ...
                if isinstance(inner_val, dict):
                    ...
                    if "oneOf" in inner:
                        ...
                        for ik, iv in inner.items():
                            if ik not in node:
                                node[ik] = iv
                        print("Unwrapped allOf->oneOf in schema property")
        ...
        for method, operation_val in methods_dict.items():
            ...
            if "requestBody" in operation:
                req_body: dict[str, Any] = (
                    cast(dict[str, Any], operation["requestBody"])
                    if isinstance(operation["requestBody"], dict)
                    else {}
                )
                ...
                if "application/json" in content:
                    if content["application/json"] == {}:
                        print(
                            f"Patching empty request body for {str(method).upper()} {path}"
                        )
