...
async def test_job_worker_performance():
    async with _make_client() as camunda:
        with open('./tests/integration/resources/job_worker_load_test_process_1.bpmn', 'rb') as f:
            ...
            deployed_resources = await camunda.create_deployment_async(body=CreateDeploymentBody(resources=[process_file]))
        ...
        alreadyRunningProcesses = camunda.search_process_instances(body=searchQuery)
        for process in alreadyRunningProcesses.items:
            await camunda.cancel_process_instance_async(body=None, process_instance_key=process.process_instance_key)
        # Start 100 instances
