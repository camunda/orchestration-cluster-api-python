...
from io_bound_workload import simulate_io_work_async # pyright: ignore[reportUnusedImport]
...
async def main():
    ...
    print(f"Deployed process definition: {process_definition_id} (Key: {process_definition_key})")

    cancel_running_process_instances(camunda, process_definition_key=process_definition_key)
    ...
    camunda.create_job_worker(config=config, callback=partial(simulate_cpu_work, duration=WORK_DURATION_SECONDS))
    ...
    for i in range(NUM_PROCESSES_TO_START):
        ...
        print(f"Started process instance {i}/{NUM_PROCESSES_TO_START}: {process_instance.process_instance_key}")
